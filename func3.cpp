...
struct Usuario {
  ...
  float indices[10][3];
	...
	string avaliacaoIMC(int pos){
		...
		if (indices[pos][1] >= 17.0 && indices[pos][1] < 18.5){
			aval = "ABAIXO DO PESO";
		}
		if (indices[pos][1] >= 18.5 && indices[pos][1] < 24.5){
			aval = "PESO NORMAL";
		}
		if (indices[pos][1] >= 24.5 && indices[pos][1] < 30){
			aval = "ACIMA DO PESO";
		}
		if (indices[pos][1] >= 30 && indices[pos][1] < 35){
			aval = "OBESIDADE I";
		}
		if (indices[pos][1] >= 35 && indices[pos][1] < 40){
	...
	string avaliacaoRCQ(int pos){
		...
		if((indices[pos][2] >= 0.80 && indices[pos][2] <= 0.85 && sexo == 'F') || (indices[pos][2] >= 0.95 && indices[pos][2] <= 1.0 && sexo == 'M')){
				aval = "MODERADO RISCO DE SAUDE";
		}
		if((indices[pos][2] > 0.85 && sexo == 'F') || (indices[pos][2] > 1.0 && sexo == 'M')){
